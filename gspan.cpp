...
void Gspan::edge_grow(GraphToTracers& g2tracers){
  if (p_count > maxGN){return;}
  ...
  for(GraphToTracers::iterator x = g2tracers.begin(); x != g2tracers.end(); ++x){
    freq[x->first].push_back(p_count);
  }
  //std::cout << support(g2tracers) << std::endl;
  //report(g2tracers);
  p_count++;  
  PairSorter b_heap;
...
const vector<Graph> readGraphs(std::istream &is) {
  ...
  int label;
  std::ofstream y_class("labels.csv");
  std::string line;
  int eid=0;
